...
function startRealTimeUpdates() {
    ...
    setInterval(fetchSoundStatus, 500);
    setInterval(fetchLockStatus, 500);
    setInterval(fetchUIStatus, 500);
    setInterval(fetchMQTTStatus, 5000);
}

// ============================================================================
// API CALLS
// ============================================================================
function fetchSoundStatus() {
    fetch('/api/sound/status')
        .then(r => r.json())
        .then(data => {
            const box = document.getElementById('sound-status-box');
            if (data.active) {
                box.style.background = '#f59e0b';
                box.style.color = '#000';
                box.innerHTML = `<div style="text-align:center"><div style="font-weight:600">🔊 Sound Detected!</div><div style="font-size:11px">${data.last_time}</div></div>`;
            } else {
                box.style.background = '#151515';
                box.style.color = '#666';
                box.innerHTML = `<div style="text-align:center"><div>🎙 Waiting for sound...</div><div style="font-size:11px">${data.last_time ? 'Last detected: ' + data.last_time : 'No sound yet'}</div></div>`;
            }
        });
}
...
function fetchMQTTStatus() {
    fetch('/api/mqtt/status')
        ...
        .then(data => {
            ...
            dot.style.background = data.active ? '#10b981' : '#dc2626';
        });
}
    ...
    fetch('/api/lock/status')
        ...
        .then(data => {
            ...
            if (data.unlocked) {
                ...
                box.style.color = '#000';
                box.innerHTML = `<div style="text-align:center"><div style="font-weight:700;font-size:15px">🔓 UNLOCKED</div><div style="font-size:11px">Locking in ${data.remaining}s...</div></div>`;
            } else {
                box.style.background = '#151515';
                box.style.color = '#fff';
                box.innerHTML = `<div style="font-weight:700;font-size:15px">🔒 LOCKED</div>`;
            }
        });
}

function fetchUIStatus() {
    fetch('/api/ui/status')
        .then(r => r.json())
        .then(data => {
            const bar = document.getElementById('ui-status-bar');
            const s = data.status;
            if (s.startsWith('WELCOME')) {
                // only show notification when status just changed to WELCOME
                if (bar.dataset.lastStatus !== s) {
                    showNotification('✅ ' + s + ' — Door Unlocked!', 'success');
                }
                bar.dataset.lastStatus = s;
                ...
                bar.style.color = '#666';
                bar.textContent = '⚪ Waiting for activation...';
            }
        });
}
...
function createLogItem(log) {
    ...
    // Format timestamp
    const timestamp = new Date(log.timestamp + 'Z');
...
function updateSystemHealth(health) {
    ...
    const diskEl = document.querySelector('.stat-box:nth-child(3) .stat-value');
    if (diskEl && health.disk) {
        diskEl.innerHTML = `${health.disk.percent}<span class="stat-unit">%</span>`;
    }
}

function updateTime() {
    const now = new Date();
    const timeStr = now.toLocaleTimeString('en-US', { hour12: false, timeZone: 'Europe/Berlin'});
