...
"""
#pi version 05/03 
#The mic module is already working and can detect sounds such as claps or finger snaps.
# Once a sound is detected, a message is sent to the Raspberry to trigger the face 
# recognition system (which previously required pressing 'c'). If the face recognition 
# is successful, an unlock message is sent to the Arduino. Since we do not have the 
# motor yet, the unlock is represented by turning on the built-in matrix LED on the 
# Arduino for 10 seconds, after 10 secs it locks again and the matrix turns off.
